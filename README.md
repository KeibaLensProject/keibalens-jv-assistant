...
- 🤝 **サブエージェント同梱** — `jra-van-spec` を `/agents` から呼び出し／委譲でき、仕様調査を**別コンテキスト**で実行（本筋の会話を汚さない）
...
## 🤝 2 つの使い方：スキル（自動発火）とサブエージェント（委譲）

本プラグインは **スキル** と **サブエージェント** の両方を同梱しており、どちらでも JRA-VAN 仕様を参照できます。

| 使い方 | 何が起きる | こんなときに |
|---|---|---|
| **スキル（自動発火）** | メインのエージェントが `jra-van-spec-skills` スキルを読み込み、その場で一次資料を参照して回答 | 通常の質問。会話の流れの中でサッと仕様を確認したいとき |
| **サブエージェント `jra-van-spec`（委譲）** | 専用サブエージェントが**別コンテキスト**で一次資料を調査し、確定した要点だけを返す | 仕様調査ログで本筋の会話を埋めたくないとき／大きめの調査を切り出して回したいとき |

### サブエージェントの呼び出し

- `/agents` の一覧に **`jra-van-spec`**（スコープ付き表記: `jra-van-spec-skills:jra-van-spec`）が表示されます。
- 明示的に委譲するときは、メインのエージェントにこう頼みます：

```text
jra-van-spec エージェントで「SE レコードの確定着順のバイト位置」を調べて
```

- 条件に合致すれば、メインのエージェントが**自動でこのサブエージェントに委譲**することもあります。
- サブエージェントは内部で `jra-van-spec-skills` スキルを使い、`references/spec/` の一次資料を開いて
  **確定した事実だけ**を返します（記憶では答えません）。

> GitHub Copilot CLI・Claude Code のどちらでも `/agents` から同じように利用できます。
...
└─ plugins/
   └─ jra-van-spec-skills/             # ← プラグイン本体（インストール時はここが配布される）
      ...
      ├─ agents/
      │  └─ jra-van-spec.agent.md       #   サブエージェント定義（GHC / Claude 共通・既定の agents/ から自動発見）
      └─ skills/
...
標準的なプラグイン マニフェスト（`.claude-plugin/marketplace.json` ＋ `.claude-plugin/plugin.json` ＋ `skills/<name>/SKILL.md` ＋ `agents/<name>.agent.md`）形式のため、
GitHub Copilot CLI と Claude Code の両方で、スキルもサブエージェントもそのまま利用できます。
